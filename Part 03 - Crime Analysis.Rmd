...
df = data %>%
  ...
  filter(OFFENSE_TYPE_ID %in% top10)%>%
...
p = ggplot(data %>% filter(CRIME_TYPE != 'OTHER'),aes(x=factor(year),y=incidents,col=OFFENSE_TYPE_ID,fill=OFFENSE_TYPE_ID))
p + geom_bar(stat='identity') + facet_wrap(~OFFENSE_TYPE_ID) + theme(legend.position="none")

p = ggplot(data %>% filter(CRIME_TYPE != 'OTHER'),aes(x=factor(OFFENSE_TYPE_ID),y=incidents,col=year,fill=year))
p + geom_bar(stat='identity') + theme(legend.position="none") + coord_flip()
...
p + geom_line() + geom_point() + theme(axis.text.x = element_text(angle = 45,hjust=1))
...
top10categories = data %>%
  group_by(OFFENSE_CATEGORY_ID) %>%
  summarise(incidents = sum(incidents)) %>%
  arrange(desc(incidents))
top10categories = top10categories$OFFENSE_CATEGORY_ID[1:nrow(top10categories)]

df = data %>%
  filter(year >= 2013) %>%
  filter(OFFENSE_CATEGORY_ID %in% top10categories) %>%
  group_by(OFFENSE_CATEGORY_ID,year,month) %>%
  summarise(incidents = sum(incidents)) %>%
  spread(OFFENSE_CATEGORY_ID,incidents)
