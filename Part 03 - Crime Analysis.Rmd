...
head(data[data$DRUG_OFFENSE_CATEGORY!='OTHER',])
data$incidents = 1
...
topCrimes = data %>%
  ...
  arrange(desc(incidents)) %>%
  top_n(15)
topCrimes = topCrimes$OFFENSE_TYPE_ID
...
p + geom_line() + geom_point() + theme(axis.text.x = element_text(angle = 45,hjust=1)) + ggtitle(paste("Annual Incidents Reported")) + labs(x = "Neighborhood")
...
p + geom_line() + theme(legend.position = 'bottom',legend.title=element_blank()) + ggtitle(expression(atop(paste("Incidents With 'Marijuana' Classification"),atop(italic('Note Spikes in Posession of Marijuana on April 20th (aka 4/20) Each Year'),"")))) + labs(x='Year by Month')
#420 is a huge day...
...
p = ggplot(df,aes(x=year,y=incidents,col=OFFENSE_TYPE_ID,fill=OFFENSE_TYPE_ID))
p + geom_line() + theme(legend.position = 'bottom',legend.title=element_blank()) + ggtitle(expression(atop(paste("Incidents With 'Marijuana' Classification"),atop(italic('Note Spikes in Posession of Marijuana on April 20th (aka 4/20) Each Year'),"")))) + labs(x='Year by Month')
```
