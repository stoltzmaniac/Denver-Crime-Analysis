...
data$DRUG = gsub("^.*?-",'',data$OFFENSE_CATEGORY_ID)
data$DRUG = gsub("-^.*?",'-',data$DRUG)

  

data$CRIME_TYPE = 'OTHER'
data$CRIME_TYPE[grepl('poss',data$OFFENSE_TYPE_ID)] = 'POSs'
...
df = data %>%
  ...
  filter(year >= 2014) %>%
  filter(month < maxMonthYTD) %>%
  group_by(DRUG,year,OFFENSE_CATEGORY_ID,OFFENSE_TYPE_ID) %>%
  summarise(incidents = sum(incidents))
df[df$DRUG == 'OTHER',]
df[is.na(df)] = 0

p = ggplot(df,aes(x=reorder(DRUG,-incidents),y=incidents,group=factor(year),col=factor(year)))
